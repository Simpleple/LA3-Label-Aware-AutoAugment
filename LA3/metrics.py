...
def class_precision(output, target, class_res):
    batch_size = target.size(0)

    _, pred = output.topk(1, 1, True, True)
    pred = pred.t()
    correct = pred.eq(target.view(1, -1).expand_as(pred)).int()
    pred = pred.squeeze(0)

    for i in range(batch_size):
        class_res[pred[i].item()][0] += correct[0, i].item()
        class_res[pred[i].item()][1] += 1

    return class_res


def class_recall(output, target, class_res):
    batch_size = target.size(0)
    ...
    for i in range(batch_size):
        class_res[target[i].item()][0] += correct[0, i].item()
        class_res[target[i].item()][1] += 1
