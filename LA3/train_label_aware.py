...
from LA3.common import get_logger, EMA, add_filehandler
from LA3.data import get_dataloaders
from LA3.lr_scheduler import adjust_learning_rate_resnet
from LA3.metrics import accuracy, Accumulator, CrossEntropyLabelSmooth
from LA3.networks import get_model, num_class
from LA3.tf_port.rmsprop import RMSpropTF
...
def train_and_eval(tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='last', save_path=None, only_eval=False, local_rank=-1, evaluation_interval=5):
    ...
    trainsampler, trainloader, validloader, testloader_ = get_dataloaders(C.get()['dataset'], C.get()['batch'], dataroot, test_ratio, split_idx=cv_fold, multinode=(local_rank >= 0), aug_p_path=args.policy_path, cls_policy_path=args.cls_policy)
    ...
    criterion_ce = criterion = CrossEntropyLabelSmooth(num_class(C.get()['dataset']), 0.1 if C.get()['dataset'].startswith('imagenet') else 0.0)
    if C.get().conf.get('mixup', 0.0) > 0.0:
        criterion = CrossEntropyMixUpLabelSmooth(num_class(C.get()['dataset']), 0.1 if C.get()['dataset'].startswith('imagenet') else 0.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy_path', type=str, default='stable_average_largest_results')
    parser.add_argument('--cls_policy', type=str, default='stable_average_largest_results/top_100_cls_policy_0.0.pkl')
